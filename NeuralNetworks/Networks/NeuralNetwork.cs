...
namespace NeuralNetworks
{
    ...
    public class NeuralNetwork
    {
        ...
        public NeuralNetwork(int numberOfInputs, LayerCharacteristic[] layersChars, bool isBiasOn = true, double min = -1, double max = 1)
        {
            ...
            Layers[0] = new Layer(Matrix<double>.Build.Dense(NumberOfInputs + inputNumberModifier, layersChars[0].NumberOfNeurons, (x,y) => (randomizer.NextDouble()*(max-min)) + min), layersChars[0].ActivationFunction); // input layer
            for (int i=1; i< NumberOfLayers; i++) 
            {
                ...
                Layers[i] = new Layer(Matrix<double>.Build.Dense(inputsAmount, numberOfNeurons, (x, y) => (randomizer.NextDouble() * (max - min)) + min), layersChars[i].ActivationFunction); // +1 becuase of bias weights.
            }
